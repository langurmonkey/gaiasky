...
- Orbital element groups refactor: We have merged orbital element groups into regular particle groups, consolidating the data model and gaining all the particle group goodies: filtering, indexing, foucsing, texturing, and shading types.
...
- Better handling of name conflicts in catalogs and datasets:
	- Add name conflicts window to inform the user of index conflicts in the current session. Fixes [#904](https://codeberg.org/gaiasky/gaiasky/issues/904).
	- Dataset updates (base data, NBG, FPR SSO, nebulae) to try to mitigate name conflicts.

- Internationalization and translations:
	- Simplified Chinese translation, by BCY-SuiCi. Part of [#885](https://codeberg.org/gaiasky/gaiasky/issues/885).
...
- User interface updates:
	- Move from static font files to `gdx-freetype` library. This enables us to support CJK languages and more. Part of [#885](https://codeberg.org/gaiasky/gaiasky/issues/885).
	- Remove monospace fonts from Gaia Sky UI and skin. They are used very sporadically and are not really needed. Part of [#885](https://codeberg.org/gaiasky/gaiasky/issues/885).
...
- Stereoscopic and anaglyph 3D:
	- Add custom anaglyph mode that uses user-defined colors. Implement custom anaglyph in backend filter. Part of [#874](https://codeberg.org/gaiasky/gaiasky/issues/874).
	- Add UI controls to settings dialog to select left and right colors in custom anaglyph mode. Part of [#874](https://codeberg.org/gaiasky/gaiasky/issues/874).
...
- Camcorder and OptFlowCam export:
	...
	- Replace `pipenv` with `uv` for dependency management for the OptFlowCam script. It is much faster. Part of [#894](https://codeberg.org/gaiasky/gaiasky/issues/894).
	- Update OptFlowCam readme with new method (not yet implemented). Part of [#894](https://codeberg.org/gaiasky/gaiasky/issues/894).
...
- Improve index conflict priority handling, and rework logging to make it more informative. Part of [#904](https://codeberg.org/gaiasky/gaiasky/issues/904).
- Use 'umbra' and 'penumbra' in Catalan translation instead of the common names 'ombra' and 'penombra'.
- Prevent smoothed distance to go to NaN in natural camera. This breaks the speed scaling computation. Fixes [#902](https://codeberg.org/gaiasky/gaiasky/issues/902).
- Adapt look-up texture path names to internal format using backslash instead of forward slash. Fixes crash in Windows. Part of [#902](https://codeberg.org/gaiasky/gaiasky/issues/902).
...
- Use component type localized names in error messages in search dialog. Fixes [#901](https://codeberg.org/gaiasky/gaiasky/issues/901).
...
- Remove `pack()` from top info interface notify method. Part of [#898](https://codeberg.org/gaiasky/gaiasky/issues/898).
...
- Use HTTPS to check internet connection with fedoraproject.org.
- Dataset manager window: expand filter input field, remove top padding in datasets list.
- Typos in objects translation files: Ophiuc**H**us and Antar**C**tica.
- Guard `keyStringToKeyCode()` execution against null strings. Fixes [#896](https://codeberg.org/gaiasky/gaiasky/issues/896).
- Light glow texture aspect ratio in stereo half-width modes (cross-eye, parallel view).
- Resolve camera path discontinuities in CatmullRom interpolation in OptFlowCam. Fixes [#895](https://codeberg.org/gaiasky/gaiasky/issues/895).
...
- Delay `GatewayServer` start (scripting) until the end of the loading pipeline, when the scene is already initialized and the Gaia Sky state is actually ready. Fixes [#893](https://codeberg.org/gaiasky/gaiasky/issues/893), [#891](https://codeberg.org/gaiasky/gaiasky/issues/891).
...
- Capture `InvalidPathException`s thrown by windows paths. Mark current location red when the path is malformed, or the location does not exist. Fixes [#887](https://codeberg.org/gaiasky/gaiasky/issues/887).
