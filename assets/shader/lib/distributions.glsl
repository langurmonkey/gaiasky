...
float noise(in vec3 st) {
    ...
    // Four corners in 3D of a tile (cube)
    float a = random(i);
    ...
    // Interpolate between the corners using the fade function (smoothstep)
    return mix(a, b, u.x) +
    (c - a) * u.y * (1.0 - u.x) +
    (d - b) * u.x * u.y * (1.0 - u.z) +
    (e - a) * u.z * (1.0 - u.x) +
    (f1 - b) * u.x * u.z * (1.0 - u.y) +
    (g - c) * u.y * u.z * (1.0 - u.x) +
    (h - d) * u.x * u.y * u.z;
}
...
float fbm(in vec3 st) {
    ...
    float value = 0.0;
    float amplitude = 0.5;
    float frequency = 1.0;  // Start at frequency 1.0, then increase with each octave

    // Loop of octaves
    for (int i = 0; i < OCTAVES; i++) {
        value += amplitude * noise(st);
        st *= 2.0;  // Increase frequency (scale the coordinates)
