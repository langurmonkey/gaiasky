...
vec2 svtTexCoords(vec2 texCoords) {
    ...
    vec2 pageCoord = indirection.rg; // red-green has the UV coordinates of the tile in the cache texture.
    float level = indirection.b * u_svtDepth; // blue channel has  level over depth.
    vec2 indirectionSize = vec2(pow(2.0, level + 1.0), pow(2.0, level));
    // Size of the buffer texture, in tiles.
    float bufferSizeInTiles = textureSize(u_svtCacheTexture, 0).x / u_svtTileSize;
