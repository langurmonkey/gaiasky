...
public class NoiseUtils {
    ...
    private static float fract(float n) {
    ...
    private static Vector3 add(Vector3 a, float n) {
        return new Vector3(a).add(n);
    }
    ...
    private static Vector3 sub(float n, Vector3 b) {
        return new Vector3(n, n, n).sub(b);
    }
    ...
    private static Vector3 scl(Vector3 a, Vector3 b) {
        return new Vector3(a).scl(b);
    }
    ...
    private static Vector3 max(Vector3 a, float n){
        return new Vector3(Math.max(a.x, n), Math.max(a.y, n), Math.max(a.z, n));
    }

    // Modulo 289, optimizes to code without divisions
    public static Vector3 mod289(Vector3 x) {
        return sub(x, scl(floor(scl(x, (1.0f / 289.0f))), 289.0f));
    }
    ...
    public static Vector3 permute(Vector3 x) {
        return mod289(scl(add(scl(x, 34.0f), 1.0f), x));
    }
    ...
    public static Vector2 rgrad2(Vector2 p, float rot) {
        ...
        return new Vector2(Math.abs(u) - 1.0f, Math.abs(Math.abs(u + 1.0f) - 2.0f) - 1.0f);
    }
    ...
    public static float psrnoise(Vector2 pos, Vector2 per, float rot) {
        ...
        Vector2 g0 = rgrad2(new Vector2(iuw.x, ivw.x), rot);
        Vector2 g1 = rgrad2(new Vector2(iuw.y, ivw.y), rot);
        Vector2 g2 = rgrad2(new Vector2(iuw.z, ivw.z), rot);

        // Gradients dot vectors to corresponding corners
        // (The derivatives of this are simply the gradients)
        Vector3 w = new Vector3(dot(g0, d0), dot(g1, d1), dot(g2, d2));

        // Radial weights from corners
        // 0.8 is the square of 2/sqrt(5), the distance from
        // a grid point to the nearest simplex boundary
        Vector3 t = sub(0.8f, new Vector3(dot(d0, d0), dot(d1, d1), dot(d2, d2)));
    ...
    //
    public static float psnoise(Vector2 pos, Vector2 per) {
