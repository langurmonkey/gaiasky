...
import gaiasky.render.gdx.loader.CubemapLoader.CubemapParameter;
...
public class OwnCubemap extends GLTexture {
    ...
    public OwnCubemap(CubemapData data, TextureFilter minFilter, TextureFilter magFilter) {
        super(GL20.GL_TEXTURE_CUBE_MAP);
        this.data = data;
        load(data, minFilter, magFilter);
        if (data.isManaged())
            addManagedCubemap(Gdx.app, this);
    }

    /** Construct a Cubemap with the specified texture files for the sides, does not generate mipmaps. */
    public OwnCubemap(FileHandle positiveX, FileHandle negativeX, FileHandle positiveY, FileHandle negativeY, FileHandle positiveZ,
    ...
    public OwnCubemap(int width, int height, int depth, Format format) {
    ...
    public OwnCubemap(TextureData positiveX, TextureData negativeX, TextureData positiveY, TextureData negativeY,
    ...
    public OwnCubemap(TextureData positiveX, TextureData negativeX, TextureData positiveY, TextureData negativeY,
            TextureData positiveZ, TextureData negativeZ, TextureFilter minFilter, TextureFilter magFilter) {
        this(new FacedCubemapData(positiveX, negativeX, positiveY, negativeY, positiveZ, negativeZ), minFilter, magFilter);
    }
    ...
    private static void addManagedCubemap(Application app, OwnCubemap cubemap) {
        ...
        if (managedCubemapArray == null)
            managedCubemapArray = new Array<OwnCubemap>();
    ...
    public void invalidateAllCubemaps(Application app) {
        ...
        if (assetManager == null) {
            for (int i = 0; i < managedCubemapArray.size; i++) {
                OwnCubemap cubemap = managedCubemapArray.get(i);
                cubemap.reload();
            }
        } else {
            ...
            // asset manager.
            Array<OwnCubemap> cubemaps = new Array<OwnCubemap>(managedCubemapArray);
            for (OwnCubemap cubemap : cubemaps) {
                ...
                if (fileName == null) {
                    ...
                    assetManager.load(fileName, OwnCubemap.class, params);
                }
            }
    ...
    public void load(CubemapData data, TextureFilter minFilter, TextureFilter magFilter) {
        if (!data.isPrepared())
            data.prepare();
        bind();
        Gdx.gl.glTexParameteri(GL20.GL_TEXTURE_CUBE_MAP, GL30.GL_TEXTURE_BASE_LEVEL, 0);
        Gdx.gl.glTexParameteri(GL20.GL_TEXTURE_CUBE_MAP, GL30.GL_TEXTURE_MAX_LEVEL, 8);
