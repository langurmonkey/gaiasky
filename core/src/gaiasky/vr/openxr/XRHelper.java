...
public final class XRHelper {
    ...
    public static <S extends Struct, T extends StructBuffer<S, T>> T fill(T buffer, int offset, int value) {
    ...
    /**
     * Applies an off-center asymmetric perspective projection transformation to the given {@link Matrix4},
     ...
     */
    public static Matrix4 applyProjectionToMatrix(Matrix4 m, XrFovf fov, float nearZ, float farZ, boolean zZeroToOne) {
        float distToLeftPlane   = (float) Math.tan(fov.angleLeft());
        float distToRightPlane  = (float) Math.tan(fov.angleRight());
        float distToBottomPlane = (float) Math.tan(fov.angleDown());
        float distToTopPlane    = (float) Math.tan(fov.angleUp());
     ...
     */
    public static XrSessionCreateInfo createGraphicsBindingOpenGL(
