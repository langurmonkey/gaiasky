...
public class EventScriptingInterface implements IScriptingInterface, IObserver {
    ...
    public void configureFrameOutput(int width,
        ...
        if (checkNum(width, 1, Integer.MAX_VALUE, "width") && checkNum(height, 1, Integer.MAX_VALUE, "height") && checkNum(fps, Constants.MIN_FPS,
                                                                                                                           Constants.MAX_FPS, "FPS")
                && checkString(directory, "directory") && checkDirectoryExists(directory, "directory") && checkString(namePrefix, "namePrefix")) {
            ...
            synchronized (focusView) {
                if (Mapper.atmosphere.has(object)) {
                    ...
                    em.post(Event.TURNING_SPEED_CMD, this,
                            (float) MathUtilsDouble.lint(20d, Constants.MIN_SLIDER, Constants.MAX_SLIDER, Constants.MIN_TURN_SPEED,
                                                         Constants.MAX_TURN_SPEED), false);
        ...
        if (checkNotNull(entity, "object") && checkNum(latitude, -90d, 90d, "latitude") && checkNum(longitude, 0d, 360d, "longitude")) {
            synchronized (focusView) {
                ...
                if (Mapper.atmosphere.has(entity)) {
                    ...
                    double turnSpeedBak = Settings.settings.scene.camera.turn;
                    em.post(Event.TURNING_SPEED_CMD, this,
                            (float) MathUtilsDouble.lint(50d, Constants.MIN_SLIDER, Constants.MAX_SLIDER, Constants.MIN_TURN_SPEED,
                    ...
                    em.post(Event.ROTATION_SPEED_CMD, this,
                            (float) MathUtilsDouble.lint(20d, Constants.MIN_SLIDER, Constants.MAX_SLIDER, Constants.MIN_ROT_SPEED,
                                                         Constants.MAX_ROT_SPEED));
                    ...
                    boolean intersects = IntersectorDouble.checkIntersectSegmentSphere(cam.pos.tov3d(aux3d3), target.tov3d(aux3d1),
                                                                                       objectPosition.tov3d(aux3d2), focusView.getRadius());
    ...
    public double[] getStarParameters(String id) {
        if (checkObjectName(id)) {
            ...
            if (Mapper.starSet.has(entity)) {
                ...
                if (sb != null) {
                    double[] rgb = sb.rgb();
                    return new double[] { sb.ra(), sb.dec(), sb.parallax(), sb.mualpha(), sb.mudelta(), sb.radvel(), sb.appMag(), rgb[0], rgb[1],
                            rgb[2] };
                }
            }
        }
    ...
    public boolean loadParticleDataset(String dsName,
        ...
        return loadParticleDataset(dsName, path, profileDecay, dArray(particleColor), colorNoise, dArray(labelColor), particleSize,
                                   dArray(sizeLimits), ct, dArray(fadeIn), dArray(fadeOut), sync);
    }
    ...
    public boolean loadParticleDataset(String dsName,
        ...
        return loadParticleDataset(dsName, path, CatalogInfoSource.SCRIPT, profileDecay, particleColor, colorNoise, labelColor, particleSize,
                                   sizeLimits, ct, fadeIn, fadeOut, sync);
    }
    ...
    private boolean loadDatasetImmediate(String dsName,
        ...
        try {
            ...
            if (checkString(dsName, "datasetName")) {
                ...
                } else if (datasetOptions == null || datasetOptions.type == DatasetLoadType.STARS
    ...
    public void addShapeAroundObject(String shapeName,
                                     ...
                                     boolean showLabel,
                                     boolean trackObject) {
        if (checkString(shapeName, "shapeName") && checkStringEnum(shapeType, Shape.class, "shape") && checkStringEnum(primitive, Primitive.class,
                                                                                                                       "primitive") && checkNum(size,
                                                                                                                                                0,
                                                                                                                                                Double.MAX_VALUE,
                                                                                                                                                "size")
    ...
    class CameraTransitionRunnable implements Runnable {
        ...
        private PathDouble<Vector3d> getPathd(Vector3d p0,
                                              double[] p1) {
