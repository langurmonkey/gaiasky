...
public class ObjectDoubleMap<K> implements Iterable<ObjectDoubleMap.Entry<K>> {
	...
	public double put (K key, float value, double defaultValue) {
		int i = locateKey(key);
		if (i >= 0) { // Existing key was found.
	...
	public double getAndIncrement (K key, double defaultValue, float increment) {
		int i = locateKey(key);
		if (i >= 0) { // Existing key was found.
			double oldValue = valueTable[i];
	...
	/** Returns the key for the specified value, or null if it is not in the map. Note this traverses the entire map and compares
	 * every value, which may be an expensive operation. */
	public @Null K findKey (double value, double epsilon) {
		K[] keyTable = this.keyTable;
		double[] valueTable = this.valueTable;
		for (int i = valueTable.length - 1; i >= 0; i--) {
			K key = keyTable[i];
	...
	public int hashCode () {
		...
		for (int i = 0, n = keyTable.length; i < n; i++) {
			K key = keyTable[i];
			if (key != null) h += key.hashCode() + NumberUtils.floatToRawIntBits((float) valueTable[i]);
		}
		return h;
	}
	...
	public boolean equals (Object obj) {
		...
		if (!(obj instanceof ObjectDoubleMap)) return false;
