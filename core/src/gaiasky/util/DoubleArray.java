...
public class DoubleArray {
	...
	public void add (double value) {
		...
		if (size == items.length) items = resize(Math.max(8, (int)(size * 1.75)));
		items[size++] = value;
	}

	public void add (double value1, double value2) {
		double[] items = this.items;
		if (size + 1 >= items.length) items = resize(Math.max(8, (int)(size * 1.75)));
		items[size] = value1;
		items[size + 1] = value2;
		size += 2;
	}
	...
	public void add (double value1, double value2, double value3) {
		double[] items = this.items;
		if (size + 2 >= items.length) items = resize(Math.max(8, (int)(size * 1.75)));
		items[size] = value1;
		items[size + 1] = value2;
		items[size + 2] = value3;
		size += 3;
	...
	public void add (double value1, double value2, double value3, double value4) {
		double[] items = this.items;
		if (size + 3 >= items.length) items = resize(Math.max(8, (int)(size * 1.8))); // 1.75 isn't enough when size=5.
		items[size] = value1;
		items[size + 1] = value2;
		items[size + 2] = value3;
		items[size + 3] = value4;
	...
	public void addAll (double[] array, int offset, int length) {
		...
		if (sizeNeeded > items.length) items = resize(Math.max(Math.max(8, sizeNeeded), (int)(size * 1.75)));
	...
	public double[] ensureCapacity (int additionalCapacity) {
		...
		if (sizeNeeded > items.length) resize(Math.max(Math.max(8, sizeNeeded), (int)(size * 1.75)));
		return items;
	}
