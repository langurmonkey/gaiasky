...
import gaiasky.util.gdx.g3d.particles.renderers.IntBillboardControllerRenderData;
...
import static gaiasky.util.gdx.g3d.particles.ParticleIntShader.AlignMode;

public class IntBillboardParticleBatch extends IntBufferedParticleBatch<IntBillboardControllerRenderData> {
	protected static final Vector3d TMP_V1 = new Vector3d(), TMP_V2 = new Vector3d(), TMP_V3 = new Vector3d(), TMP_V4 = new Vector3d(),
		TMP_V5 = new Vector3d(), TMP_V6 = new Vector3d();
	protected static final Matrix3d TMP_M3 = new Matrix3d();
	// Attributes
	...
	private class RenderablePool extends Pool<IntRenderable> {
		public RenderablePool () {
		}

		@Override
		public IntRenderable newObject () {
	...
	private final Array<IntRenderable> renderables;
	...
	public IntBillboardParticleBatch(AlignMode mode, boolean useGPU, int capacity, BlendingAttribute blendingAttribute,
									 DepthTestAttribute depthTestAttribute) {
		super(IntBillboardControllerRenderData.class);
	...
	public IntBillboardParticleBatch(AlignMode mode, boolean useGPU, int capacity) {
		this(mode, useGPU, capacity, null, null);
	}

	public IntBillboardParticleBatch() {
		this(AlignMode.Screen, false, 100);
	}

	public IntBillboardParticleBatch(int capacity) {
	...
	protected IntRenderable allocRenderable () {
		IntRenderable renderable = new IntRenderable();
		renderable.meshPart.primitiveType = GL20.GL_TRIANGLES;
		renderable.meshPart.offset = 0;
		renderable.material = new Material(this.blendingAttribute, this.depthTestAttribute,
			TextureAttribute.createDiffuse(texture));
		renderable.meshPart.mesh = new IntMesh(false, MAX_VERTICES_PER_MESH, MAX_PARTICLES_PER_MESH * 6, currentAttributes);
	...
	protected IntShader getShader (IntRenderable renderable) {
		IntShader shader = useGPU ? new ParticleIntShader(renderable, new ParticleIntShader.Config(mode)) : new DefaultIntShader(renderable);
		shader.init();
		return shader;
	}

	private void allocShader () {
		IntRenderable newRenderable = allocRenderable();
	...
	private void clearRenderablesPool () {
		...
		for (int i = 0, free = renderablePool.getFree(); i < free; ++i) {
			IntRenderable renderable = renderablePool.obtain();
	...
	// CPU
	// Required
	private static void putVertex (float[] vertices, int offset, Vector3d p, float u, float v, float r, float g, float b, float a) {
		// Position
		vertices[offset + CPU_POSITION_OFFSET] = (float) p.x;
		vertices[offset + CPU_POSITION_OFFSET + 1] = (float) p.y;
		vertices[offset + CPU_POSITION_OFFSET + 2] = (float) p.z;
		// UV
	...
	private void fillVerticesGPU (int[] particlesOffset) {
		int tp = 0;
		for (IntBillboardControllerRenderData data : renderData) {
	...
	private void fillVerticesToViewPointCPU (int[] particlesOffset) {
		...
		for (IntBillboardControllerRenderData data : renderData) {
			...
			for (int p = 0, c = data.controller.particles.size; p < c; ++p, ++tp) {
				...
				float cosRotation = rotationChannel.data[rotationOffset + ParticleChannels.CosineOffset];
				float sinRotation = rotationChannel.data[rotationOffset + ParticleChannels.SineOffset];
				Vector3d look = TMP_V3.set(camera.position).sub(px, py, pz).nor(), // normal
	...
	private void fillVerticesToScreenCPU (int[] particlesOffset) {
		Vector3d look = TMP_V3.set(camera.direction).scl(-1), // normal
			right = TMP_V4.set(camera.up).crs(look).nor(), // tangent
			up = TMP_V5.set(camera.up);
	...
	public void getRenderables (Array<IntRenderable> renderables, Pool<IntRenderable> pool) {
		for (IntRenderable renderable : this.renderables)
