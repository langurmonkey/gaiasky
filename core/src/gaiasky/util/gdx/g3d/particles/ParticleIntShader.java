...
public class ParticleIntShader extends BaseIntShader {
	...
	protected static Bits implementedFlags = Bits.from(BlendingAttribute.Type | TextureAttribute.Diffuse);
	...
	private Bits materialMask;
	private long vertexMask;
	protected final Config config;
	/** Material attributes which are not required but always supported. */
	private final static Bits optionalAttributes = Bits.from(IntAttribute.CullFace | DepthTestAttribute.Type);

	public ParticleIntShader(final IntRenderable renderable) {
		this(renderable, new Config());
	}

	public ParticleIntShader(final IntRenderable renderable, final Config config) {
		this(renderable, config, createPrefix(renderable, config));
	}

	public ParticleIntShader(final IntRenderable renderable, final Config config, final String prefix) {
		this(renderable, config, prefix, config.vertexShader != null ? config.vertexShader : getDefaultVertexShader(),
			config.fragmentShader != null ? config.fragmentShader : getDefaultFragmentShader());
	}

	public ParticleIntShader(final IntRenderable renderable, final Config config, final String prefix, final String vertexShader,
                             final String fragmentShader) {
		this(renderable, config, new ExtShaderProgram(prefix + vertexShader, prefix + fragmentShader));
	}

	public ParticleIntShader(final IntRenderable renderable, final Config config, final ExtShaderProgram shaderProgram) {
		this.config = config;
		...
		if (!config.ignoreUnimplemented && !implementedFlags.and(materialMask).equals(materialMask))
			throw new GdxRuntimeException("Some attributes not implemented yet (" + materialMask + ")");

		// Global uniforms
		//register(DefaultIntShader.Inputs.viewTrans, DefaultIntShader.Setters.viewTrans);
		register(DefaultIntShader.Inputs.projViewTrans, DefaultIntShader.Setters.projViewTrans);
		register(DefaultIntShader.Inputs.projTrans, DefaultIntShader.Setters.projTrans);
		register(Inputs.screenWidth, Setters.screenWidth);
		register(DefaultIntShader.Inputs.cameraUp, Setters.cameraUp);
		register(Inputs.cameraRight, Setters.cameraRight);
		register(Inputs.cameraInvDirection, Setters.cameraInvDirection);
		register(DefaultIntShader.Inputs.cameraPosition, Setters.cameraPosition);

		// Object uniforms
		register(DefaultIntShader.Inputs.diffuseTexture, DefaultIntShader.Setters.diffuseTexture);
	}
	...
	public boolean canRender (final IntRenderable renderable) {
		...
		return materialMask.equals(renderableMask.or(optionalAttributes)) && (vertexMask == renderable.meshPart.mesh.getVertexAttributes().getMaskWithSizePacked());
	}
	...
	public boolean equals (Object obj) {
		return (obj instanceof ParticleIntShader) && equals((ParticleIntShader)obj);
	}

	public boolean equals (ParticleIntShader obj) {
		return (obj == this);
	}
