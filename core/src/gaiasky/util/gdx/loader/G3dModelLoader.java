...
public class G3dModelLoader extends IntModelLoader<IntModelLoader.IntModelParameters> {
    ...
    @Override
    public IntModelData loadModelData(FileHandle fileHandle, IntModelParameters parameters) {
        return parseModel(fileHandle);
    }

    public IntModelData parseModel(FileHandle handle) {
        JsonValue json = reader.parse(handle);
        IntModelData model = new IntModelData();
    ...
    private void parseMeshes(IntModelData model, JsonValue json) {
        ...
        if (meshes != null) {
            ...
            for (JsonValue mesh = meshes.child; mesh != null; mesh = mesh.next) {
                IntModelMesh jsonMesh = new IntModelMesh();
            ...
            for (JsonValue material = materials.child; material != null; material = material.next) {
                ...
                if (textures != null) {
                    for (JsonValue texture = textures.child; texture != null; texture = texture.next) {
                        ...
                        jsonTexture.fileName = materialDir + (materialDir.isEmpty() || materialDir.endsWith("/") ? "" : "/")
                                + fileName;
    ...
    private IntModelNode parseNodesRecursively(JsonValue json) {
        IntModelNode jsonNode = new IntModelNode();
        ...
        if (children != null) {
            jsonNode.children = new IntModelNode[children.size];
    ...
    private void parseAnimations(IntModelData model, JsonValue json) {
