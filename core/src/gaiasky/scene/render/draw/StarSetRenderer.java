...
public class StarSetRenderer extends PointCloudQuadRenderer implements IObserver {
    ...
    protected void renderObject(ExtShaderProgram shaderProgram, IRenderable renderable) {
        ...
        var highlight = Mapper.highlight.get(render.entity);
        var datasetDesc = Mapper.datasetDescription.get(render.entity);

        synchronized (render) {
            if (!set.disposed) {
                boolean hlCmap = highlight.isHighlighted() && !highlight.isHlplain();
                if (!inGpu(render)) {
                    ...
                    for (int i = 0; i < n; i++) {
                        if (utils.filter(i, set, datasetDesc) && set.isVisible(i)) {
                            ...
                            for (int vert = 0; vert < 4; vert++) {
                                ...
                                if (hlCmap) {
                                    // Color map
                                    double[] color = cmap.colormap(highlight.getHlcmi(), highlight.getHlcma().get(particle), highlight.getHlcmmin(), highlight.getHlcmmax());
                                    tempVerts[curr.vertexIdx + curr.colorOffset] = Color.toFloatBits((float) color[0], (float) color[1], (float) color[2], 1.0f);
                                } else {
                                    // Plain
                                    tempVerts[curr.vertexIdx + curr.colorOffset] = utils.getColor(i, set, highlight);
                                }

                                // SIZE
                                if (highlight.isHlAllVisible() && highlight.isHighlighted()) {
                                    tempVerts[curr.vertexIdx + sizeOffset] = Math.max(10f, (float) (particle.size() * Constants.STAR_SIZE_FACTOR) * utils.highlightedSizeFactor(highlight, datasetDesc));
                                } else {
                                    tempVerts[curr.vertexIdx + sizeOffset] = (float) (particle.size() * Constants.STAR_SIZE_FACTOR) * utils.highlightedSizeFactor(highlight, datasetDesc);
                                }
                ...
                if (curr != null) {
                    ...
                    triComponent.alphaSizeBr[1] = triComponent.starPointSize * 1e6f * utils.highlightedSizeFactor(highlight, datasetDesc);
